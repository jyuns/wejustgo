...
/**
    SET_BOOK_MARKED(state, payload) {
        ...
        const chatRef = firebase.database().ref('chatRooms/' + state._chatCurrentRoom.chatRoomId + '/chatMember/' + user)
        const userRef = firebase.database().ref('users/' + user + "/" + state._chatCurrentRoom.chatRoomId)
...
export const actions = {
    ...
    async POPULAR_CHAT_LOAD({commit, state}, payload) {
        firebase.database().ref('currentChat').limitToLast(5).once('value', async (snapshot) => {
            snapshot.forEach( (value) => {
                state._popularChatList.push(value.val())
            })
        })
    },
    ...
    async CHAT_LOAD({commit, state}, payload) {
        ...
        state._chatTriggerRef = await firebase.database().ref('chatRooms/' + payload + '/chatMsg').limitToLast(5)
...
/**
        const memberCount = await firebase.database().ref('chatRooms/' + payload + '/chatMember').once('value', async (snapshot) => {
    ...
    async CHAT_MORE_LOAD({commit, state}, payload) {
        ...
        const result = await firebase.database().ref('chatRooms/' + state._chatCurrentRoom.chatRoomId + '/chatMsg')
